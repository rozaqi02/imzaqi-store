...
export default function Testimonials() {
  ...
  const filtered = useMemo(() => items.filter(item => {
    ...
    return !q.trim() || caption.toLowerCase().includes(q.trim().toLowerCase());
  }), [items, q, captionMode]);
  const shown = filtered.slice(0, limit);
  const active = activeIdx >= 0 ? filtered[activeIdx] : null;
  const resetFilters = () => { setQ(""); setCaptionMode("all"); };

  useEffect(() => { setLimit(18); setActiveIdx(-1); }, [captionMode, q, view]);
          ...
          {loading ? <div className="reviews-grid" aria-hidden="true">{Array.from({ length: 6 }, (_, i) => <div key={i} className="reviews-skeleton" />)}</div>
            ...
            : <><div className={view === "grid" ? "reviews-grid" : "reviews-masonry"}>
              {shown.map((item, idx) => <button className="reviews-card" type="button" key={item.id} onClick={() => setActiveIdx(idx)} aria-label={`Buka testimoni ${idx + 1}${item.caption ? `: ${item.caption}` : ""}`}>
                ...
                <span className="reviews-cardCopy"><span className="reviews-cardLabel">CERITA {String(idx + 1).padStart(2, "0")}<ArrowUpRight size={16} /></span><strong>{item.caption?.trim() || "Testimoni pelanggan"}</strong><small>Ketuk untuk melihat lengkap</small></span>
              </button>)}
      ...
      {active && createPortal(
        <div className="reviews-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) closeViewer(); }}>
          <div ref={dialogRef} className="reviews-lightbox" role="dialog" aria-modal="true" aria-labelledby="reviews-viewer-title">
            ...
            <div className="reviews-viewerFoot"><p>{active.caption?.trim() || "Testimoni pelanggan"}</p>{filtered.length > 1 && <div className="reviews-viewerNav"><button className="reviews-iconButton" type="button" onClick={() => setActiveIdx(i => (i - 1 + filtered.length) % filtered.length)} aria-label="Testimoni sebelumnya"><ChevronLeft size={22} /></button><button className="reviews-iconButton" type="button" onClick={() => setActiveIdx(i => (i + 1) % filtered.length)} aria-label="Testimoni berikutnya"><ChevronRight size={22} /></button></div>}</div>
          </div>
        </div>, document.body)}
    </div>
  );
